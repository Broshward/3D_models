...
count=264
#obj = 'cylinder(1,d=1)'
text='ln(0)=D '#'FUAZRKXPHNIGJCYSTBEMLWOD'
obj = 'rotate(90) translate([0,-1,0]) text("%s", size = %f, font = "%s:style=Bold", valign = "center")'
line= 'rotate(90) translate([0,-%s,0]) cube([%s,%s,1])'
a=0.0
...
#    l_r = asin((dr/2)/step)*2*180/pi
    ...
    if r>50: 
        obj1='linear_extrude(1) '+obj %(text[i%len(text)],r/4,"Times New Roman" if i%len(text)!=(len(text)-2) else "New")
    else:
        obj1 = line %(r/16,r/8,r/4)
    ...
    step=r/5
    spiral_step+=(asin(h/r)/(2*pi)*r)*0.21
